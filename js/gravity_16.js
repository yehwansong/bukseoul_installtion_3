...
$('document').ready(function(){
    if(window.location.hash){
      for (var i = 0; i < $('.value_input').length; i++) {
        ...
        for (var i =  9; i < value_array_n.length; i++) {
            if(value_array_n[i]  == 1){
                document.getElementById("checkbox_"+(i-8)).checked = true;}
      }
    }
})
}else{
    ...
    // red
        ...
        ['rec', 1,   9, 6.2,    3,  1, red_sec],
        ['rec',10,   9, 2.2,    2,  1, red_sec],
        ['rec', 7,  14, 5.8,  2.5,  1, red_sec],
